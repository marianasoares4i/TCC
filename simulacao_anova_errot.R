#### DADOS SIMULADOS MODELO ANOVA ERRO T ####
rm(list=ls())
library(dplyr)
library(lattice)
library(brms)
...
lambda<-rgamma(n,shape=ni/2,scale=2/ni)
## erro N(0,1)
epsilon<- rnorm(n,0,1)
...
##### DADOS PARA FAZER PREVISAO NO MODELO #####
...
## matrix com 0 e 1 para alpha das linhas
...
#### MODELO LOG ANOVA BAYESIANO ERRO T ####

## colocando como fator
Claims$dev<-as.factor(Claims$dev)

## Model matrix com todos os coeficientes
X_a <- as.data.frame(model.matrix(~ dev, data=Claims,
                                  contrasts.arg = list(dev = contrasts(Claims$dev, contrasts = FALSE))))[,-1]
X_b<-as.data.frame( model.matrix(~originf, data=Claims,
                                 contrasts.arg = list(originf = contrasts(Claims$originf, contrasts = FALSE))))[,-1]
Y <- Claims$inc.paid.k
K<- ncol(X_b)
N<- nrow(X_b)
anov<-lm(Claims$inc.paid.k~dev+originf, data=Claims,contrasts=list(originf="contr.sum",dev="contr.sum"))
summary(anov)

anova_t <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/anova_errot_simulacao.stan",
...
traceplot(anova_t,pars="ni")
traceplot(anova_t,pars="sigma2")
lambda_prev<-extract(anova_t)$lambda_prev
reserva<-(extract(anova_t)$reserva)
zij_prev<-(extract(anova_t)$muij_new)
quantile(reserva)
View(summary(anova_t))
resultado<-as.matrix(anova_t)
lambda_prev<-apply(lambda_prev, 2, median) # e 1 ou 2 ?
coefs <-apply(resultado, 2, median)
coefs <-apply(resultado, 2, quantile)
View(coefs)

dados_simulados<- list(alpha=alpha,beta=beta,epsilon=epsilon,lambda=lambda,muij=muij,
                       zij=zij,Claims,allClaims,triangulo_total=triangulo_total)
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_simulacao_anova_t.rds")
saveRDS(dados_simulados,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/dados_simulados_anova_t.rds")


#### MODELO ANOVA ERRO T DADOS CHOY ####
n<-18
...
###### DADOS PARA FAZER PREVISAO DO MODELO ######
...
anova_t <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/anova_errot_choy.stan",
                data=list(X_alpha=X_a,X_beta=X_b,Y=Y,K_alpha=K,K_beta=K,N=N,X_alpha_prev=X_a_prev,
                          X_beta_prev=X_b_prev, K_alpha_prev=K_prev,N_prev=N_prev,K_beta_prev=K_prev),
                warmup=10000,iter=40000,thin=50,chains=2, control=list(adapt_delta=0.99),
                cores = getOption("mc.cores", 2))  

teste<-extract(anova_t)$log_lik
...
waic(extract(anova_t)$log_lik)

traceplot(anova_t,pars=c("ni","sigma2","alpha","beta"))
traceplot(anova_t,pars="sigma2")
lambda_prev<-extract(anova_t)$lambda_prev
reserva<-(extract(anova_t)$reserva)
zij_prev<-apply((extract(anova_t)$y_new),2,median)
...
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_choy_anova_t.rds")

#### MODELO ANOVA ERRO NORMAL DADOS VERRAL ####
n<-10
## dados choy
...
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_verral_anova_errot.rds")
...
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_cote_anova_errot.rds")


#### MODELO ANOVA ERRO NORMAL DADOS MOTOR ####
n<-10
Claims <- data.frame(originf = factor(rep(1981:1990, 10:1)),
                     dev=sequence(n:1),
                     inc.paid=c( 33810,45318,46549,35206,23360,12502,6602,3373,2373,778,
                                 37663,51771,40998,29496,12669,11204,5785,4220,1910,
                                 40630,56318,56182,32473,15828,8409,7120,1125,
                                 40475,49697,39313,24044,13156,12595,2908,
                                 37127,50983,34154,25455,19421,5728,
                                 41125,53302,40289,39912,6650,
                                 57515,67881,86734,18109,
                                 61553,132208,20923,
                                 112103,33250,
                                 37554))
...
###### DADOS PARA FAZER PREVISAO ######
dados_prev<-matrix(1,nrow=9,ncol=9)

# zerando o que esta abaixo da diagonal principal que e conhecido
dados_prev[row(dados_prev) <= n - col(dados_prev) + 1]<-NA

allClaims <- data.frame(originf =factor(sort(rep(1:9, 9))),
                        dev=rep(1:9,9),
                        inc.paid= as.vector(t(dados_prev)))
allClaims<-allClaims %>% na.omit()
allClaims$dev<-as.factor(allClaims$dev)
...
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_motor_anova_errot.rds")
...
saveRDS(anova_t,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_soa_anova_errot.rds")

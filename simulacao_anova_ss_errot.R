##### DADOS SIMULADOS SS MODELO ANOVA ERRO T #####
rm(list=ls())
library(dplyr)
library(lattice)
library(brms)
...
lambda<-rgamma(400,shape=ni/2,scale=2/ni)
...
#### MODELO SS LOG ANOVA BAYESIANO ERRO T DADOS SIMULADOS ####
...
anova_errot_ss <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/ss_anova_errot_simulacao.stan",
                        data=data_list,
...
summary(anova_errot_ss)
resultado<-as.matrix(anova_errot_ss)
coefs <-apply(resultado, 2, median)
coefs <-apply(resultado, 2, quantile, probs=c(0.025,0.975))
traceplot(anova_errot_ss,pars=c("u","sigma_u","ni"))
traceplot(anova_errot_ss,pars=c("sigma_beta","sigma_u","sigma_alpha"))

mu<-extract(anova_errot_ss)$u
sigma2<-extract(anova_errot_ss)$sigma_u
ni<-extract(anova_errot_ss)$ni
...
dados_simulados<- list(alpha=alpha,beta=beta,epsilon=epsilon,lambda=lambda,muij=muij,zij=zij,Claims,triangulo_total=triangulo_total)
saveRDS(anova_errot_ss,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/simulacao_anova_ss_errot.rds")
saveRDS(dados_simulados,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/dados_simulados_anova_ss_errot.rds")

#### MODELO ANOVA SS ERRO T DADOS CHOY ####
n<-18
...
#### MODELO SS LOG ANOVA BAYESIANO ERRO T DADOS CHOY ####
...
## rodando o modelo no stan
anova_errot_ss <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/ss_anova_errot.stan",
                        data=data_list,
                        warmup=150000,iter=300000,thin=50,
                        chains=2, control=list(adapt_delta=0.99),cores = getOption("mc.cores", 2))  
...
teste<-rstan::extract(anova_errot_ss)$log_lik
soma<-apply(teste,MARGIN=1,FUN=sum)
# lppd <-sum(log(colMeans(exp(log_lik))))
lppd <-sum(log(mean(exp(soma))))
p_waic <- sum(stats::var(soma))
waic <- -2*lppd +2*p_waic
waic(rstan::extract(anova_errot_ss)$log_lik)

res<-rstan::extract(anova_errot_ss)$reserva
y_pre<-extract(anova_errot_ss)$y_pred
y_pred2 <-apply(y_pre, 2, median)
y_new<-extract(anova_errot_ss)$zij
y_new2 <-apply(y_new, 2, median)
quantile((res))

## salvando resultado do modelo
saveRDS(anova_errot_ss,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_choy_ss_errot.rds")


#### MODELO ANOVA SS ERRO T DADOS VERRAL ####
n<-10
## dados verral
...
#### MODELO SS LOG ANOVA BAYESIANO ERRO T DADOS VERRAL ####
## PASSANDO LOG NOS PAGAMENTOS
Claims$logClaims<-log(Claims$inc.paid.k)
...
teste<-extract(anova_errot_ss)$log_lik
soma<-apply(teste,MARGIN=1,FUN=sum)
# lppd <-sum(log(colMeans(exp(log_lik))))
lppd <-sum(log(mean(exp(soma))))
p_waic <- sum(stats::var(soma))
waic <- -2*lppd +2*p_waic
waic(extract(anova_errot_ss)$log_lik)

res<-extract(anova_errot_ss)$reserva
y_pre<-extract(anova_errot_ss)$y_pred
y_pred2 <-apply(y_pre, 2, median)
y_new<-extract(anova_errot_ss)$zij
y_new2 <-apply(y_new, 2, median)
...
## SALVANDO RESULTADO DO MODELO 
saveRDS(anova_errot_ss,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_verral_ss_errot.rds")


#### MODELO ANOVA SS ERRO T DADOS COTE ####
n<-10
...
## SALVANDO RESULTADO DO MODELO 
saveRDS(anova_errot_ss,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_cote_ss_errot.rds")


#### MODELO ANOVA SS ERRO T DADOS SOA ####
n<-12
...
## SALVANDO RESULTADO DO MODELO 
saveRDS(anova_errot_ss,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_t/result_soa_ss_errot.rds")

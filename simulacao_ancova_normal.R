#### MODELO ANCOVA ERRO NORMAL COM DADOS SIMULADOS ####
...
## gerando o coeficiente dos efeitos das linhas/anos N(0,4)
alpha<-rnorm(1,0,2)
...
muij<-mu+matrix(rep(alpha,400),ncol=i)*matrix(rep((1:20),20),nrow=i)+t(matrix(rep(beta,20),ncol=i))
...
anov<-lm(Claims$inc.paid.k~anos+dev, data=Claims)
...
ancova_normal <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/ancova_normal_simulado.stan",
...
View(summary(ancova_normal))
traceplot(ancova_normal)
resultado<-as.matrix(ancova_normal)
coefs <-apply(resultado, 2, median)

dados_simulados<- list(alpha=alpha,beta=beta,epsilon=epsilon,muij=muij,zij=zij,Claims,allClaims,triangulo_completo=triangulo_completo)
saveRDS(ancova_normal,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_normal//result_simulacao_ancova_normal.rds")
saveRDS(dados_simulados,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/dados_simulados_ancova_normal.rds")


#### MODELO ANCOVA ERRO NORMAL DADOS CHOY ####
n<-18
...
## rodando o modelo no stan
ancova_normal <- stan("/home/mariana/Pessoal/Modelos_tcc_teste_validation/MODELOS USADOS/ancova_normal_choy.stan",
                      data=list(X_alpha=X_a,X_beta=X_b,Y=Y,K_beta=K,N=N,X_alpha_prev=X_b_prev,X_beta_prev=X_a_prev,
                                K_beta_prev=K_prev,N_prev=N_prev),
                      warmup=10000,iter=40000,thin=50,
                      chains=2, control=list(adapt_delta=0.99),cores = getOption("mc.cores", 2))

modelo<-extract(ancova_normal)
alpha<-extract(ancova_normal)$alpha
beta<-extract(ancova_normal)$beta
boxplot(alpha)
boxplot(beta)
summary(ancova_normal)

teste<-extract(ancova_normal)$log_lik
...
waic(extract(ancova_normal)$log_lik)
...
saveRDS(ancova_normal,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_normal//result_choy_ancova_normal.rds")



#### MODELO ANCOVA ERRO NORMAL DADOS VERRAL ####
n<-10
## dados choy
...
resultado<-as.matrix(ancova_normal)
coefs <-apply(resultado, 2, median)
y_new<-(extract(ancova_normal)$y_new)
View(extract(ancova_normal)$y_new)
y_novo<-apply(y_new,MARGIN=2,FUN=median)
saveRDS(ancova_normal,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_normal/result_verral_ancova_normal.rds")


#### MODELO ANCOVA ERRO NORMAL DADOS COTE ####
n<-10
...
View(extract(ancova_normal)$y_new)
y_novo<-apply(y_new,MARGIN=2,FUN=median)
saveRDS(ancova_normal,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_normal/result_cote_ancova_normal.rds")



#### MODELO ANCOVA ERRO NORMAL DADOS SOA ####
n<-12
...
View(extract(ancova_normal)$y_new)
y_novo<-apply(y_new,MARGIN=2,FUN=median)
saveRDS(ancova_normal,file="/home/mariana/Pessoal/Modelos_tcc_teste_validation/modelos_erro_normal/result_soa_ancova_normal.rds")
